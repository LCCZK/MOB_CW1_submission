...
class ParticleFilterLocalisationNode(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().error("""Problem getting a map. Check that you have an activated map_server
                                    ...
                                    """)
            sys.exit(1)
        self.get_logger().info("Map received. %d X %d, %f px/m." %(ocuccupancy_map.info.width, ocuccupancy_map.info.height, ocuccupancy_map.info.resolution))
        self._particle_filter.set_map(ocuccupancy_map)
        
        self._laser_subscriber = self.create_subscription(LaserScan, "/base_scan", self._laser_callback, 1)
        self._initial_pose_subscriber = self.create_subscription(PoseWithCovarianceStamped, "/initialpose", self._initial_pose_callback, 1)
        self._odometry_subscriber = self.create_subscription(Odometry, "/odom", self._odometry_callback, 1)

    def _initial_pose_callback(self, pose):
        self.get_logger().info("/initialpose recived")
    ...
    def _laser_callback(self, scan):
        ...
        """
        self.get_logger().info("/base_scan recived")
